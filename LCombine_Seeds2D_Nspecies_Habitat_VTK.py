...
for n in range(0,xprunus.shape[0],1):
    ...
    an = xprunus[n,0,:,:].T+xprunus[n,1,:,:].T
    IM = an.reshape(Np,Np,1)
    imageToVTK(savefile +'/seeds00' + str(n), cellData = {"seeds" : IM})

    an = xprunus[n,-1,:,:].T
    IM = an.reshape(Np,Np,1)
    imageToVTK(savefile +'/adults00' + str(n), cellData = {"adults" : IM})
